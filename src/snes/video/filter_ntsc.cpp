...
//to do: defer initialization?
  adjust(0, 0, 0, 0, 0, bool(config::snes.ntsc_merge_fields), 0); //snes->color_lookup_table);
}
...
int const out_height = height;
  ...
  if(!scanline_widths) {
    if(width == 256) {
      snes_ntsc_blit(ntsc, data, pitch, burst, out_width, out_height, output, output_pitch);
    } else {
      snes_ntsc_blit_hires(ntsc, data, pitch, burst, out_width, out_height, output, output_pitch);
    }
  } else {
  ...
  int line       = 0;
    ...
    while(1) {
      ...
      uint16 const *in = (uint16*)((char*)data + pitch * run_start);
      uint16 *out      = (uint16*)((char*)output + output_pitch * run_start);
      int height       = line - run_start;
      int line_burst   = (burst + run_start) % 3;
        if(run_width == 256) {
          snes_ntsc_blit(ntsc, in, pitch, line_burst, out_width, height, out, output_pitch);
        } else {
          snes_ntsc_blit_hires(ntsc, in, pitch, line_burst, out_width, height, out, output_pitch);
        }
