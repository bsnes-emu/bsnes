...
void Memory::updateVideo(unsigned cycleCounter) {
	return display.update(cycleCounter);
}

unsigned Memory::lyCounter(unsigned cycleCounter) {
	return display.getLyReg(cycleCounter);
}

void Memory::updateInput() {
	unsigned joypadId = 0x0F;
...
// 	printf("mem[0x%X] = 0x%X\n", P, data);
	...
	case 0x00:
		...
		if(memoryInterface) memoryInterface->joypWrite(data & 0x20, data & 0x10);
		break;
	case 0x01:
		update_irqEvents(cycleCounter);
		break;
...
bool Memory::loadROM(const bool forceDmg) {
	if (!memoryInterface) return 1;
	...
	Gambatte::MemoryBuffer romBuffer = memoryInterface->loadRomData();
	if(romBuffer.size < 0x150) return 1;

	{
		unsigned char *header = (unsigned char*)romBuffer.data;
	...
	rombanks = pow2ceil(romBuffer.size / 0x4000);
	//std::printf("rombanks: %u\n", romBuffer.size / 0x4000);
	...
	//rom.rewind();
	//rom.read(reinterpret_cast<char*>(romdata[0]), (romBuffer.size / 0x4000) * 0x4000ul);
	memcpy(romdata[0], romBuffer.data,
      std::min( romBuffer.size, (unsigned)((romBuffer.size / 0x4000) * 0x4000ul) )
    );
	...
	std::memset(romdata[0] + (romBuffer.size / 0x4000) * 0x4000ul, 0xFF, (rombanks - romBuffer.size / 0x4000) * 0x4000ul);
...
/*		std::ifstream file((sbp + ".sav").c_str(), std::ios::binary | std::ios::in);
		...
		Gambatte::MemoryBuffer buffer = memoryInterface->loadRamData();
		if (buffer.data) {
			memcpy(rambankdata, buffer.data, std::min(buffer.size, (unsigned)(rambanks * 0x2000ul)));
			enforce8bit(rambankdata, rambanks * 0x2000ul);
		}
	}

	if (rtcRom && memoryInterface) {
/*		std::ifstream file((sbp + ".rtc").c_str(), std::ios::binary | std::ios::in);
		...
		Gambatte::MemoryBuffer buffer = memoryInterface->loadRtcData();
		if (buffer.data && buffer.size >= 4) {
			unsigned long basetime = 0;
			uint8_t *data = (uint8_t*)buffer.data;

			basetime = basetime << 8 | (data[0]);
			basetime = basetime << 8 | (data[1]);
			basetime = basetime << 8 | (data[2]);
			basetime = basetime << 8 | (data[3]);
...
/*		std::ofstream file((sbp + ".sav").c_str(), std::ios::binary | std::ios::out);
		...
		Gambatte::MemoryBuffer buffer = memoryInterface->saveRamData(rambanks * 0x2000ul);
		if (buffer.data) {
			memcpy(buffer.data, rambankdata, std::min(buffer.size, (unsigned)(rambanks * 0x2000ul)));
		}
	}
...
/*		std::ofstream file((sbp + ".rtc").c_str(), std::ios::binary | std::ios::out);
		...
		Gambatte::MemoryBuffer buffer = memoryInterface->saveRtcData();
		if (buffer.data && buffer.size >= 4) {
			const unsigned long basetime = rtc.getBaseTime();
			uint8_t *data = (uint8_t*)buffer.data;
