...
void input_event(uint16_t code) {
  ...
  if(window_main.focused()) {
    ...
    if(code == inputuidebugger.export_memory.code) export_memory();
    if(code == inputuidebugger.toggle_cputrace.code) toggle_cputrace();
    if(code == inputuidebugger.toggle_cputracemask.code) toggle_cputracemask();
    if(code == inputuidebugger.toggle_smptrace.code) toggle_smptrace();
    if(code == inputuidebugger.toggle_smptracemask.code) toggle_smptracemask();
  }
}
