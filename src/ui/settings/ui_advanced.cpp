...
int pos = list.get_selection();
  ...
  if(pos >= 0 && pos < config::config().list.size()) {
  string default_;
    config::config().list[pos]->get_default(default_);
    desc.set_text(string() << "(default = " << default_ << ")\n" << config::config().list[pos]->description);
  string value_;
    config::config().list[pos]->get(value_);
...
void AdvancedWindow::read_config(uint pos, string &data) {
  data = "?\t?\t?";
  if(pos >= config::config().list.size()) return;
  ...
  string default_;
  config::config().list[pos]->get_default(default_);
  if(value_ != default_) { strcat(name, " (*)"); }
  data = string()
    << name << "\t"
    << (config::config().list[pos]->type == setting::string_type ? "string" : "integral") << "\t"
    << value_;
}
...
void AdvancedWindow::update(uint pos, const char *data) {
  ...
  string default_;
  config::config().list[pos]->get_default(default_);
  config::config().list[pos]->set(data ? data : (const char*)default_);
  string value_;
  config::config().list[pos]->get(value_);
  edit_val.set_text(value_);
  read_config(pos, value_);
  list.set_item(pos, value_);
...
uint y = 0;
  ...
  for(int i = 0; i < config::config().list.size(); i++) {
  string val;
    read_config(i, val);
    list.add_item(val);
  }
