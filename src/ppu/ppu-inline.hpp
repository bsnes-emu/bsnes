...
void PPUcounter::vcounter_tick() {
  ...
  if((system.region() == System::NTSC && status.interlace == false && status.vcounter == 262)
  || (system.region() == System::NTSC && status.interlace == true  && status.vcounter == 263)
  || (system.region() == System::NTSC && status.interlace == true  && status.vcounter == 262 && status.field == 1)
  || (system.region() == System::PAL  && status.interlace == false && status.vcounter == 312)
  || (system.region() == System::PAL  && status.interlace == true  && status.vcounter == 313)
  || (system.region() == System::PAL  && status.interlace == true  && status.vcounter == 312 && status.field == 1)
  ) {
    status.vcounter = 0;
...
uint16 PPUcounter::hdot() const {
  if(system.region() == System::NTSC && status.interlace == false && vcounter() == 240 && field() == 1) {
    return (hcounter() >> 2);
  } else {
...
uint16 PPUcounter::lineclocks() const {
  if(system.region() == System::NTSC && status.interlace == false &&  vcounter() == 240 &&  field() == 1) return 1360;
  return 1364;
}
