...
bool DSPDebugger::property(unsigned id, string &name, string &value) {
  ...
  if(id == n++) { name = "Key On"; value = string::printf("0x%.2x", key_on()); return true; }
  if(id == n++) { name = "Key Off"; value = string::printf("0x%.2x", key_off()); return true; }
  ...
  if(id == n++) { name = "Pitch Modulation Enable"; value = string::printf("0x%.2x", pitch_modulation_enable()); return true; }
  if(id == n++) { name = "Noise Enable"; value = string::printf("0x%.2x", noise_enable()); return true; }
  if(id == n++) { name = "Echo Enable"; value = string::printf("0x%.2x", echo_enable()); return true; }
  ...
  for(unsigned i = 0; i < 8; i++) {
    if(id == n++) {
      ...
      value = string::printf("0x%.2x", echo_filter_coefficient(i));
      return true;
    }
  }
  ...
  for(unsigned i = 0; i < 8; i++) {
    ...
    if(id == n++) { name = "Pitch Height"; value = string::printf("0x%.4x", voice_pitch_height(i)); return true; }
